...
BANKS = [
    dict(bank_id=1,  name="Сбербанк",          bik="044525225", korr_account="30101810400000000225", address="г. Москва, ул. Вавилова, 19",          hidden=0),
    dict(bank_id=2,  name="ВТБ",               bik="044525187", korr_account="30101810700000000187", address="г. Москва, Дегтярный пер., 11",         hidden=0),
    dict(bank_id=3,  name="Газпромбанк",        bik="044525823", korr_account="30101810200000000823", address="г. Москва, ул. Намёткина, 16",          hidden=0),
    dict(bank_id=4,  name="Альфа-Банк",         bik="044525593", korr_account="30101810200000000593", address="г. Москва, ул. Каланчёвская, 27",       hidden=0),
    dict(bank_id=5,  name="Россельхозбанк",     bik="044525111", korr_account="30101810200000000111", address="г. Москва, Гагаринский пер., 3",        hidden=0),
    dict(bank_id=6,  name="Тинькофф Банк",      bik="044525974", korr_account="30101810145250000974", address="г. Москва, 1-й Волоколамский пр., 10",  hidden=0),
    dict(bank_id=7,  name="Райффайзенбанк",     bik="044525700", korr_account="30101810200000000700", address="г. Москва, ул. Троицкая, 17/1",         hidden=0),
    dict(bank_id=8,  name="Росбанк",            bik="044525256", korr_account="30101810000000000256", address="г. Москва, ул. Маши Порываевой, 34",    hidden=0),
    dict(bank_id=9,  name="Промсвязьбанк",      bik="044525555", korr_account="30101810600000000555", address="г. Москва, Смирновская ул., 10/22",     hidden=0),
    dict(bank_id=10, name="МКБ",               bik="044525659", korr_account="30101810600000000659", address="г. Москва, Луков пер., 2/1 с.1",        hidden=0),
    dict(bank_id=11, name="Совкомбанк",         bik="043469743", korr_account="30101810200000000743", address="г. Кострома, пр. Текстильщиков, 46",    hidden=0),
    dict(bank_id=12, name="Банк «Открытие»",    bik="044525297", korr_account="30101810300000000297", address="г. Москва, ул. Летниковская, 2/4",      hidden=0),
    dict(bank_id=13, name="Уралсиб",            bik="044525790", korr_account="30101810100000000790", address="г. Москва, ул. Ефремова, 8",            hidden=0),
    dict(bank_id=14, name="Ситибанк",           bik="044525202", korr_account="30101810300000000202", address="г. Москва, Гасетный пер., 17-19/1",    hidden=0),
    dict(bank_id=15, name="Абсолют Банк",       bik="044525976", korr_account="30101810500000000976", address="г. Москва, Цветной б-р, 18",            hidden=1),
]
...
CURRENCIES = [
    dict(currency_id=1, shortname="руб", fullname="Российский рубль",   code=643, gender=1),
...
DOCUMENT_TYPES = [
    dict(document_type_id=1, name="Заказ-наряд",      shortname="ЗН",  flag=1, payment_direction=0),
    dict(document_type_id=2, name="Счёт",             shortname="СЧ",  flag=2, payment_direction=1),
    dict(document_type_id=3, name="Расходная накладная", shortname="РН", flag=3, payment_direction=0),
    dict(document_type_id=4, name="Приходная накладная", shortname="ПН", flag=4, payment_direction=1),
    dict(document_type_id=5, name="Акт выполненных работ", shortname="АВР", flag=5, payment_direction=0),
]
...
DIRECTORY_REGISTRIES_NOMENCLATURE = [
    dict(directory_registry_id=400, metatable_id=13, create_user_id=1, change_user_id=1),
    dict(directory_registry_id=401, metatable_id=13, create_user_id=1, change_user_id=1),
    dict(directory_registry_id=402, metatable_id=13, create_user_id=1, change_user_id=1),
    dict(directory_registry_id=403, metatable_id=13, create_user_id=1, change_user_id=1),
    dict(directory_registry_id=404, metatable_id=13, create_user_id=1, change_user_id=1),
]
...
def seed(model, pk_attr: str, rows: list[dict], label: str) -> None:
    ...
    inserted = sum(
        1 for row in rows
        if row[pk_attr] not in existing and not model.objects.create(**row) is None
    )
...
TABLES = [
    ...
    (DirectoryRegistry,    "directory_registry_id", DIRECTORY_REGISTRIES,        "directory_registry (org)"),
    (Organization,         "organization_id",       ORGANIZATIONS,               "organization"),
    (ClientTree,           "client_tree_id",        CLIENT_TREES,                "client_tree"),
    (DirectoryRegistry,    "directory_registry_id", DIRECTORY_REGISTRIES_CLIENT, "directory_registry (client)"),
    (Client,               "client_id",             CLIENTS,                     "client"),
    (DirectoryRegistry,    "directory_registry_id", DIRECTORY_REGISTRIES_MODEL,  "directory_registry (model)"),
    (ModelDetail,          "model_detail_id",       MODEL_DETAILS,               "model_detail"),
    (ModelLink,            "model_link_id",         MODEL_LINKS,                 "model_link"),
    (ShopNomenclatureTree, "shop_nomenclature_tree_id", SHOP_NOMENCLATURE_TREES, "shop_nomenclature_tree"),
    (DirectoryRegistry,    "directory_registry_id", DIRECTORY_REGISTRIES_NOMENCLATURE, "directory_registry (nomenclature)"),
